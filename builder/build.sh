...
if wget $CACHE_URL -O /tmp/cache.tar 2> /dev/null ; then
	...
	tar -xzf /tmp/cache.tar -C /tmp 2> /dev/null
	if [ $? -eq 0 ];then
		echo "       Extraction complete"
	else
...
tar -czf /tmp/build.tar /app 2> /dev/null
if [ $? -eq 0 ];then
...
echo "       Pushing build"
mc cp --quiet /tmp/build.tar s3/$APP_UPLOAD_PATH > /dev/null
if [ $? -eq 0 ];then
	echo "       Push complete"
else
...
tar -czf /tmp/cache.tar /tmp/cache 2> /dev/null
if [ $? -eq 0 ];then
	echo "       Compression complete"
else
...
echo "       Pushing cache"
mc cp --quiet /tmp/cache.tar s3/$CACHE_UPLOAD_PATH > /dev/null
if [ $? -eq 0 ];then
	echo "       Push complete"
else
